...
router.get('/', function(req, res){
    ...
    WHERE student_id = '${id}'`;
    let query2 = `SELECT AVG(grand_total) AS average_total FROM 3rd_term
    WHERE student_id = '${id}'
    `;
    let query3 =`SELECT count(student_id) AS class_total FROM student_table
    WHERE class_id = '${class_id}'
    `;
    let query4 = `SELECT sum(grand_total) AS sum_total FROM 3rd_term
    WHERE student_id = '${id}'
    `;
